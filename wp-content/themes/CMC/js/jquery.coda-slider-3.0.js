	...
	var Slider = {
		...
		events: function(){
			...
			$(self.sliderId).parent().addClass("arrows");
                            ...
                            $($(self.sliderId).parent()).find('[class^=coda-nav-left]').addClass('disabled');
                            else
                             $($(self.sliderId).parent()).find('[class^=coda-nav-left]').removeClass('disabled');   
                        if(self.currentTab === (self.panelCount-1))
                            //$($(self.sliderId).parent()).find('[class^=coda-nav-right]').hide();
                            $($(self.sliderId).parent()).find('[class^=coda-nav-right]').addClass('disabled');
                            else
                            $($(self.sliderId).parent()).find('[class^=coda-nav-right]').removeClass('disabled');
                             //$($(self.sliderId).parent()).find('[class^=coda-nav-right]').show();
		},
